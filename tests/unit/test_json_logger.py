"""Unit tests for JSON event logger."""
...
class TestJSONEventLogger:
    """Test cases for JSONEventLogger class."""
    ...
    def sample_event(self) -> Event:
        ...
        return Event(
            ...
            llm_description="Person detected walking",
    ...
    def test_init(self, config: SystemConfig) -> None:
        ...
        logger = JSONEventLogger(config)
        assert logger.config == config
        assert logger.logger is not None

    def test_log_event_creates_directory_structure(
        self, logger: JSONEventLogger, sample_event: Event
    ) -> None:
    ...
    def test_log_event_calls_atomic_append(
        self, logger: JSONEventLogger, sample_event: Event
    ) -> None:
            ...
            with patch("pathlib.Path.mkdir"):
                with patch.object(logger, "_atomic_append", return_value=True) as mock_append:
                    ...
                    try:
                        ...
                        assert str(target_file).endswith("data/events/2025-11-10/events.json")
    ...
    def test_log_event_handles_atomic_append_failure(
        self, logger: JSONEventLogger, sample_event: Event
    ) -> None:
    ...
    def test_log_event_logs_performance_warning(
        self, logger: JSONEventLogger, sample_event: Event
    ) -> None:
    ...
    def test_atomic_append_handles_file_operation_errors(self) -> None:
        ...
        invalid_path = Path("/nonexistent/deep/path/test.json")
        content = '{"test": "data"}\n'
    ...
    def test_log_event_json_format_validation(
        self, logger: JSONEventLogger, sample_event: Event
    ) -> None:
        """Test that logged content is valid JSON Lines format."""
        captured_content = None

        def capture_append(target_file: Path, content: str) -> bool:
            nonlocal captured_content
            captured_content = content
            # Simulate successful write by ensuring parent directory exists
            target_file.parent.mkdir(parents=True, exist_ok=True)
            target_file.write_text(content)
            return True

        with patch.object(logger, "_atomic_append", side_effect=capture_append):
            result = logger.log_event(sample_event)

            assert result is True
            assert captured_content is not None
            assert captured_content.endswith("\n")

            # Verify JSON is valid
            json_line = captured_content.strip()
            parsed = json.loads(json_line)

            # Verify required fields are present
            assert parsed["event_id"] == sample_event.event_id
            assert parsed["camera_id"] == sample_event.camera_id
            assert parsed["llm_description"] == sample_event.llm_description
            assert "timestamp" in parsed
            assert "detected_objects" in parsed

    def test_log_event_error_logging(self, logger: JSONEventLogger, sample_event: Event) -> None:
